...
final class ExerciseSystemEdgeCaseTests: XCTestCase {
    ...
    func testEquipmentCaseSensitivity() {
        ...
        // When
        let correctResults = exerciseService.getExercises(for: .arms, availableEquipment: correctEquipment)
        let incorrectResults1 = exerciseService.getExercises(for: .arms, availableEquipment: ["dumbbells"])
        let incorrectResults2 = exerciseService.getExercises(for: .arms, availableEquipment: ["DUMBBELLS"])
    ...
    func testEmptyEquipmentStrings() {
        ...
        // When
        let results = exerciseService.getExercises(for: .chest, availableEquipment: availableEquipment)
        ...
        for exercise in results {
            XCTAssertTrue(exercise.requiredEquipment.isSubset(of: validEquipment),
    ...
    func testLargeEquipmentSets() {
        ...
        // When
        let results = exerciseService.getExercisesWithEquipmentHints(for: .fullBody, availableEquipment: largeEquipmentSet)
    ...
    func testAllMuscleGroupsCovered() {
        ...
        for muscleGroup in allMuscleGroups {
            let exercises = exerciseService.getExercises(for: muscleGroup, availableEquipment: noEquipment)
    ...
    func testMuscleGroupConsistency() {
        ...
        for muscleGroup in MuscleGroup.allCases {
            let firstCall = exerciseService.getExercises(for: muscleGroup, availableEquipment: testEquipment)
            let secondCall = exerciseService.getExercises(for: muscleGroup, availableEquipment: testEquipment)
            
            // Then - Results should be consistent
            XCTAssertEqual(firstCall.count, secondCall.count,
                          "Results should be consistent for \(muscleGroup.rawValue)")
            
            let firstIds = Set(firstCall.map { $0.id })
            let secondIds = Set(secondCall.map { $0.id })
    ...
    func testDifficultyLevelFiltering() {
        ...
        for difficulty in DifficultyLevel.allCases {
            let exercises = exerciseService.getExercises(for: difficulty, availableEquipment: noEquipment)
            XCTAssertGreaterThan(exercises.count, 0,
    ...
    func testDifficultyProgression() {
        ...
        // When
        let beginnerExercises = exerciseService.getExercises(for: .beginner, availableEquipment: fullEquipment)
        let intermediateExercises = exerciseService.getExercises(for: .intermediate, availableEquipment: fullEquipment)
        let advancedExercises = exerciseService.getExercises(for: .advanced, availableEquipment: fullEquipment)
        ...
        let beginnerBodyweight = beginnerExercises.filter { $0.requiredEquipment.isEmpty }
        let advancedBodyweight = advancedExercises.filter { $0.requiredEquipment.isEmpty }
        
        XCTAssertGreaterThanOrEqual(beginnerBodyweight.count, advancedBodyweight.count,
    ...
    func testExerciseVariationsConsistency() {
        // Given
        let exercisesWithVariations = exerciseService.getAllExercises().filter { !$0.variations.isEmpty }
        ...
        for exercise in exercisesWithVariations {
            ...
            for variation in exercise.variations {
                XCTAssertNotEqual(variation.name, exercise.name,
                                 "Variation should have different name from parent exercise")
            }
            
            // Variations should have unique IDs
            let variationIds = exercise.variations.map { $0.id }
            let uniqueIds = Set(variationIds)
            XCTAssertEqual(variationIds.count, uniqueIds.count,
                          "Variations should have unique IDs")
            
            // Difficulty modifiers should be reasonable
            for variation in exercise.variations {
                XCTAssertTrue(variation.difficultyModifier >= -2 && variation.difficultyModifier <= 2,
    ...
    func testSortingStability() {
        ...
        let results1 = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: equipment)
        let results2 = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: equipment)
        let results3 = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: equipment)
        ...
        for i in 0..<results1.count {
            XCTAssertEqual(results1[i].exercise.id, results2[i].exercise.id,
                          "Exercise order should be stable")
            XCTAssertEqual(results2[i].exercise.id, results3[i].exercise.id,
                          "Exercise order should be stable")
            XCTAssertEqual(results1[i].isAvailable, results2[i].isAvailable,
    ...
    func testAvailabilityGrouping() {
        ...
        // When
        let results = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: partialEquipment)
        ...
        let availableCount = results.filter { $0.isAvailable }.count
        let unavailableCount = results.filter { !$0.isAvailable }.count
    ...
    func testMemoryUsageWithLargeResults() {
        ...
        // When
        for muscleGroup in MuscleGroup.allCases {
            let results = exerciseService.getExercisesWithEquipmentHints(
                for: muscleGroup,
        ...
        let totalResults = allResults.flatMap { $0 }
    ...
    func testRepeatedOperationsPerformance() {
        ...
        measure {
            for _ in 0..<100 {
                _ = exerciseService.getExercisesWithEquipmentHints(for: .arms, availableEquipment: equipment)
            }
        }
    }
    ...
    func testExerciseIdUniqueness() {
        ...
        // When
        let allIds = allExercises.map { $0.id }
    ...
    func testExerciseNameUniqueness() {
        ...
        // When
        let allNames = allExercises.map { $0.name }
    ...
    func testExerciseDataImmutability() {
        // Given
        let exercise1 = exerciseService.getAllExercises().first!
        let exercise2 = exerciseService.getExercise(by: exercise1.id)!
    ...
    func testVeryLargeLimitRequests() {
        ...
        Task {
            let recentExercises = await exerciseService.getRecentlyUsedExercises(limit: 10000)
            let mostUsedExercises = await exerciseService.getMostUsedExercises(limit: 10000)
