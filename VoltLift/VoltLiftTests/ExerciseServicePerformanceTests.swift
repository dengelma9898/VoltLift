...
final class ExerciseServicePerformanceTests: XCTestCase {
    ...
    func testExerciseFiltering_PerformanceWithLargeEquipmentSet() {
        ...
        measure {
            for muscleGroup in MuscleGroup.allCases {
                _ = self.exerciseService.getExercises(for: muscleGroup, availableEquipment: largeEquipmentSet)
            }
        }
    }
    ...
    func testExerciseSearch_Performance() {
        ...
        measure {
            for id in exerciseIds {
                _ = self.exerciseService.getExercise(by: id)
            }
        }
    }
    ...
    func testMemoryUsage_WithMultipleFilterOperations() {
        ...
        for _ in 0 ..< 100 {
            for muscleGroup in MuscleGroup.allCases {
                let exercises = self.exerciseService.getExercises(for: muscleGroup, availableEquipment: equipment)
