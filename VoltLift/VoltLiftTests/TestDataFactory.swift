...
class TestDataFactory {
    ...
    func createEquipmentSelection() -> [EquipmentItem] {
        ...
        self.createdEquipmentIds.formUnion(equipment.map(\.id))
        return equipment
    }
    ...
    func createEquipmentItem(
        ...
        self.createdEquipmentIds.insert(id)
        return item
    }
    ...
    func modifyEquipmentSelection(_ originalEquipment: [EquipmentItem]) -> [EquipmentItem] {
        originalEquipment.map { equipment in
    ...
    func createWorkoutPlan(
        ...
        let exercises = self.createExercisesForPlan(count: exerciseCount, planName: name)
        ...
        self.createdPlanIds.insert(plan.id)
        return plan
    }
    ...
    func createMultipleWorkoutPlans(
        ...
        (1 ... count).map { index in
            let createdDate = Date().addingTimeInterval(-Double(index * 3_600)) // Spread over hours
            let lastUsedDate = index % 4 == 0 ? Date().addingTimeInterval(-Double(index * 1_800)) : nil

            return self.createWorkoutPlan(
    ...
    func createWorkoutPlansForEquipment(_ equipment: [EquipmentItem]) -> [WorkoutPlanData] {
        let selectedEquipment = equipment.filter(\.isSelected)

        guard !selectedEquipment.isEmpty else {
            return [self.createWorkoutPlan(name: "Bodyweight Only Plan")]
        }
        ...
        for (category, categoryEquipment) in equipmentByCategory {
            ...
            let exercises = categoryEquipment.flatMap { equipment in
                self.createExercisesForEquipment(equipment)
            }
    ...
    func createSpecializedWorkoutPlan(
        ...
        let (name, exercises) = self.generateSpecializedPlanContent(type: type, difficulty: difficulty)
        let plan = WorkoutPlanData(
            id: UUID(),
            name: name,
            exercises: exercises,
    ...
    func createExercisesForPlan(count: Int, planName: String) -> [ExerciseData] {
        ...
        let exercises = (0 ..< count).map { index in
            ...
            self.createdExerciseIds.insert(exerciseId)
            return exercise
        }
    ...
    func createPerformanceTestData() -> (equipment: [EquipmentItem], plans: [WorkoutPlanData]) {
        let equipment = self.createLargeEquipmentSet(count: 100)
        let plans = self.createMultipleWorkoutPlans(count: 50, exercisesPerPlan: 8)
    ...
    func createStressTestData() -> (equipment: [EquipmentItem], plans: [WorkoutPlanData]) {
        let equipment = self.createLargeEquipmentSet(count: 500)
        let plans = self.createMultipleWorkoutPlans(count: 200, exercisesPerPlan: 12)
        return (equipment, plans)
    }
    ...
    func createEdgeCaseTestData() -> (equipment: [EquipmentItem], plans: [WorkoutPlanData]) {
        ...
        equipment.append(self.createEquipmentItem(id: "empty-test", name: "", category: "", isSelected: false))

        // Equipment with special characters
        equipment.append(self.createEquipmentItem(
            id: "special-chars",
            name: "Equipment with émojis 🏋️‍♂️",
            category: "Special",
            isSelected: true
        ))
        ...
        equipment.append(self.createEquipmentItem(id: "long-name", name: longName, category: "Long", isSelected: true))
        ...
        self.createdPlanIds.insert(emptyPlan.id)

        // Plan with many exercises
        let manyExercisesPlan = self.createWorkoutPlan(name: "Many Exercises Plan", exerciseCount: 50)
        plans.append(manyExercisesPlan)

        // Plan with extreme values
        let extremeExercise = self.createExercise(
        ...
        self.createdPlanIds.insert(extremePlan.id)
    ...
    func cleanup() async {
        self.createdEquipmentIds.removeAll()
        self.createdPlanIds.removeAll()
        self.createdExerciseIds.removeAll()
    }

    /// Returns the count of created test items for verification
    func getCreatedItemCounts() -> (equipment: Int, plans: Int, exercises: Int) {
        (
            equipment: self.createdEquipmentIds.count,
            plans: self.createdPlanIds.count,
            exercises: self.createdExerciseIds.count
        )
    }
        ...
        case .strength:
            let exercises = [
                createExercise(
                    name: "Squats",
                    sets: 4,
                    reps: Int(8 * difficultyMultiplier),
                    weight: 135.0 * difficultyMultiplier
                ),
                self.createExercise(
                    name: "Deadlifts",
                    sets: 4,
                    reps: Int(6 * difficultyMultiplier),
                    weight: 185.0 * difficultyMultiplier
                ),
                self.createExercise(
                    name: "Bench Press",
                    sets: 4,
                    reps: Int(8 * difficultyMultiplier),
                    weight: 115.0 * difficultyMultiplier
                ),
                self.createExercise(
                    name: "Overhead Press",
                    sets: 3,
                    reps: Int(10 * difficultyMultiplier),
                    weight: 75.0 * difficultyMultiplier
                )
            ]
        ...
        case .cardio:
            let exercises = [
                ...
                self.createExercise(
                    ...
                    sets: 3,
                    reps: Int(20 * difficultyMultiplier),
                    restTime: 30
                ),
                self.createExercise(name: "Jumping Jacks", sets: 3, reps: Int(30 * difficultyMultiplier), restTime: 30),
                self.createExercise(name: "High Knees", sets: 3, reps: Int(25 * difficultyMultiplier), restTime: 30)
            ]
        ...
        case .functional:
            let exercises = [
                createExercise(
                    ...
                    sets: 3,
                    reps: 1,
                    weight: 50.0 * difficultyMultiplier,
                    restTime: 90
                ),
                self.createExercise(
                    name: "Turkish Get-ups",
                    sets: 3,
                    reps: Int(5 * difficultyMultiplier),
                    weight: 20.0 * difficultyMultiplier
                ),
                self.createExercise(name: "Bear Crawl", sets: 3, reps: Int(10 * difficultyMultiplier), restTime: 60),
                self.createExercise(
                    name: "Single Leg Deadlift",
                    sets: 3,
                    reps: Int(8 * difficultyMultiplier),
                    weight: 25.0 * difficultyMultiplier
                )
            ]
...
extension TestDataFactory {
    ...
    func validateWorkoutPlans(_ plans: [WorkoutPlanData]) -> [String] {
        ...
        for plan in plans {
            ...
            let actualOrder = plan.exercises.map(\.orderIndex).sorted()
