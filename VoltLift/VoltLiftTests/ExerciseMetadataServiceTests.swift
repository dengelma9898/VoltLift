...
final class ExerciseMetadataServiceTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        service = ExerciseMetadataService(context: context)
    }
    ...
    func testUpdatePersonalNotes_WithEmptyString_SetsToNil() async throws {
        ...
        await service.updateLastUsed(for: exerciseId, name: exerciseName)
        await service.updatePersonalNotes(for: exerciseId, notes: "Some notes")
        
        // When
        await service.updatePersonalNotes(for: exerciseId, notes: "")
    ...
    func testGetRecentlyUsedExercises_ReturnsInCorrectOrder() async throws {
        ...
        try await Task.sleep(nanoseconds: 1_000_000) // 1ms
        await service.updateLastUsed(for: exercise2Id, name: "Exercise 2")
        
        try await Task.sleep(nanoseconds: 1_000_000) // 1ms
        await service.updateLastUsed(for: exercise3Id, name: "Exercise 3")
    ...
    func testGetRecentlyUsedExercises_RespectsLimit() async throws {
        // Given
        for i in 0..<5 {
            await service.updateLastUsed(for: UUID(), name: "Exercise \(i)")
        }
