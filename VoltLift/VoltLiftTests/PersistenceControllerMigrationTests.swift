...
final class PersistenceControllerMigrationTests: XCTestCase {
    ...
    override func setUp() {
        ...
        testStoreURL = tempDirectory.appendingPathComponent("TestStore_\(UUID().uuidString).sqlite")
    ...
    override func tearDown() {
        // Clean up test files
        if let testStoreURL = testStoreURL {
    ...
    func testValidateDataIntegrity_WithValidData_Succeeds() throws {
        ...
        createValidTestData(context: context)
        try context.save()
        
        // When: Validating data integrity
        // Then: Should not throw
        XCTAssertNoThrow(try testPersistenceController.validateDataIntegrity())
    }
    ...
    func testValidateUserEquipment_WithInvalidEquipmentId_ThrowsError() throws {
        ...
        XCTAssertThrowsError(try testPersistenceController.validateDataIntegrity()) { error in
            guard let migrationError = error as? PersistenceController.MigrationError,
                  case .validationFailed(let message) = migrationError else {
    ...
    func testValidateUserEquipment_WithFutureDateAdded_ThrowsError() throws {
        ...
        equipment.name = "Test Equipment"
        equipment.isSelected = true
        equipment.dateAdded = Date().addingTimeInterval(86400) // Tomorrow
        try context.save()
    ...
    func testValidatePlanExercise_WithNegativeValues_ThrowsError() throws {
        ...
        XCTAssertThrowsError(try testPersistenceController.validateDataIntegrity()) { error in
            ...
            XCTAssertTrue(message.contains("invalid numeric values"), "Error message should mention invalid numeric values")
        }
    }
    ...
    func testValidateExerciseMetadata_WithFutureLastUsed_ThrowsError() throws {
        ...
        metadata.lastUsed = Date().addingTimeInterval(86400) // Tomorrow
    ...
    func testDetectAndHandleCorruption_WithValidData_ReturnsFalse() {
        ...
        let hasCorruption = testPersistenceController.detectAndHandleCorruption()
    ...
    func testDetectAndHandleCorruption_WithCorruptedData_ReturnsTrue() {
        ...
        let context = testPersistenceController.container.viewContext
        createCorruptedTestData(context: context)
        try? context.save()
    ...
    func testValidateDataIntegrity_Performance() {
        // Given: Large dataset
        let context = testPersistenceController.container.viewContext
        createLargeValidDataset(context: context)
        try? context.save()
        
        // When: Measuring validation performance
        measure {
            try? testPersistenceController.validateDataIntegrity()
        }
    }
    ...
    func testDetectAndHandleCorruption_Performance() {
        ...
        let context = testPersistenceController.container.viewContext
        createLargeDatasetWithCorruption(context: context)
        try? context.save()
        
        // When: Measuring corruption handling performance
        measure {
            _ = testPersistenceController.detectAndHandleCorruption()
        }
    }
