...
final class ExerciseSystemValidationTests: XCTestCase {
    ...
    func testAllExercises_HaveValidSFSymbolNames() {
        ...
        for exercise in allExercises {
            XCTAssertFalse(exercise.sfSymbolName.isEmpty,
            ...
            let isValidSymbol = validSFSymbolPatterns.contains(exercise.sfSymbolName) ||
                               ...
                               exercise.sfSymbolName.allSatisfy { $0.isLetter }
            
            XCTAssertTrue(isValidSymbol,
    ...
    func testAllExercises_HaveReasonableInstructionLength() {
        ...
        for exercise in allExercises {
            XCTAssertGreaterThan(exercise.instructions.count, 0,
                               "Exercise '\(exercise.name)' should have at least one instruction")
            XCTAssertLessThan(exercise.instructions.count, 20,
                             "Exercise '\(exercise.name)' should not have excessive instructions")
            
            for instruction in exercise.instructions {
                XCTAssertGreaterThan(instruction.count, 5,
    ...
    func testAllExercises_HaveConsistentDifficultyProgression() {
        ...
        for exercise in exercisesWithVariations {
            for variation in exercise.variations {
                ...
                XCTAssertFalse(variation.description.isEmpty,
    ...
    func testAllExercises_HaveValidEquipmentRequirements() {
        ...
        for exercise in allExercises {
            for equipment in exercise.requiredEquipment {
                XCTAssertTrue(validEquipment.contains(equipment),
    ...
    func testBodyweightExercises_RequireNoEquipment() {
        ...
        for exercise in bodyweightExercises {
            XCTAssertTrue(exercise.requiredEquipment.isEmpty,
    ...
    func testDifficultyLevelCoverage_IsBalanced() {
        ...
        for difficulty in DifficultyLevel.allCases {
            ...
            XCTAssertGreaterThan(exercisesForDifficulty.count, 0,
                               "Difficulty level '\(difficulty.rawValue)' should have exercises")
            
            // Beginner exercises should have more bodyweight options
            if difficulty == .beginner {
                let beginnerBodyweight = exercisesForDifficulty.filter { $0.requiredEquipment.isEmpty }
                XCTAssertGreaterThan(beginnerBodyweight.count, 0,
    ...
    func testExerciseNames_AreUnique() {
        ...
        XCTAssertEqual(exerciseNames.count, uniqueNames.count,
    ...
    func testExerciseVariationIds_AreUnique() {
        ...
        for exercise in allExercises {
            allVariationIds.append(contentsOf: exercise.variations.map { $0.id })
        }
        
        let uniqueVariationIds = Set(allVariationIds)
        XCTAssertEqual(allVariationIds.count, uniqueVariationIds.count,
    ...
    func testExerciseService_IntegratesCorrectlyWithCatalog() {
        ...
        let serviceExercises = exerciseService.getAllExercises()
        let catalogExercises = EnhancedExerciseCatalog.allExercises
        
        XCTAssertEqual(serviceExercises.count, catalogExercises.count,
                      "Service should return same count as catalog")
        
        let serviceIds = Set(serviceExercises.map { $0.id })
        let catalogIds = Set(catalogExercises.map { $0.id })
        
        XCTAssertEqual(serviceIds, catalogIds,
    ...
    func testExerciseDisplayItem_CalculatesAvailabilityCorrectly() {
        ...
        for exercise in allExercises {
            ...
            XCTAssertEqual(displayItem.isAvailable, expectedAvailability,
            ...
            XCTAssertEqual(displayItem.missingEquipment, expectedMissing,
    ...
    func testRequirement1_1_AllExercisesDisplayedRegardlessOfEquipment() {
        ...
        let displayItems = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: noEquipment)
    ...
    func testRequirement3_1_ExpandedExerciseDatabase() {
        ...
        XCTAssertGreaterThanOrEqual(allExercises.count, 40,
    ...
    func testRequirement4_1_HealthKitCompatibility() {
        ...
        for exercise in allExercises {
            ...
            XCTAssertEqual(legacyExercise.name, exercise.name,
                          "Legacy conversion should preserve name")
            XCTAssertEqual(legacyExercise.muscleGroup.rawValue, exercise.muscleGroup.rawValue,
