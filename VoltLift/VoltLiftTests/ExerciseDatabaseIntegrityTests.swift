...
final class ExerciseDatabaseIntegrityTests: XCTestCase {
    ...
    func testExerciseDatabase_HasMinimumRequiredExercises() {
        ...
        // Then
        XCTAssertGreaterThanOrEqual(allExercises.count, minimumExpectedCount,
    ...
    func testExerciseDatabase_HasExercisesForAllMuscleGroups() {
        ...
        // When
        let representedMuscleGroups = Set(allExercises.map { $0.muscleGroup })
        
        // Then
        for muscleGroup in allMuscleGroups {
            XCTAssertTrue(representedMuscleGroups.contains(muscleGroup),
    ...
    func testExerciseDatabase_HasBodyweightExercisesForAllMuscleGroups() {
        ...
        let bodyweightMuscleGroups = Set(bodyweightExercises.map { $0.muscleGroup })
        
        // Then
        for muscleGroup in primaryMuscleGroups {
            XCTAssertTrue(bodyweightMuscleGroups.contains(muscleGroup),
    ...
    func testExerciseDatabase_HasExercisesForAllDifficultyLevels() {
        ...
        // When
        let representedDifficulties = Set(allExercises.map { $0.difficulty })
        
        // Then
        for difficulty in allDifficultyLevels {
            XCTAssertTrue(representedDifficulties.contains(difficulty),
    ...
    func testAllExercises_HaveRequiredProperties() {
        ...
        for exercise in allExercises {
            ...
            XCTAssertFalse(exercise.safetyTips.isEmpty,
                          "Exercise '\(exercise.name)' should have safety tips")
            XCTAssertFalse(exercise.targetMuscles.isEmpty,
    ...
    func testAllExercises_HaveValidSafetyTips() {
        ...
        for exercise in allExercises {
            ...
            for safetyTip in exercise.safetyTips {
                XCTAssertFalse(safetyTip.isEmpty,
    ...
    func testExerciseVariations_HaveValidProperties() {
        ...
        for exercise in allExercises {
            for variation in exercise.variations {
                ...
                XCTAssertFalse(variation.sfSymbolName.isEmpty,
    ...
    func testAllExercises_HaveValidSFSymbolNames() {
        ...
        for exercise in allExercises {
            ...
            XCTAssertTrue(isCommonSymbol || followsNamingConvention,
    ...
    func testDifficultyDistribution_HasBeginnerFriendlyOptions() {
        ...
        let beginnerMuscleGroups = Set(beginnerExercises.map { $0.muscleGroup })
        
        // Then
        let primaryMuscleGroups: [MuscleGroup] = [.chest, .back, .shoulders, .arms, .legs, .core]
        for muscleGroup in primaryMuscleGroups {
            XCTAssertTrue(beginnerMuscleGroups.contains(muscleGroup),
        ...
        measure {
            for _ in 0..<iterations {
                for muscleGroup in MuscleGroup.allCases {
                    _ = exerciseService.getExercises(for: muscleGroup, availableEquipment: availableEquipment)
                }
            }
        }
    }
    ...
    func testExerciseDatabase_IsConsistentAcrossMultipleCalls() {
        // Given
        let firstCall = exerciseService.getAllExercises()
        
        // When
        let secondCall = exerciseService.getAllExercises()
        let thirdCall = exerciseService.getAllExercises()
        ...
        let thirdIds = Set(thirdCall.map { $0.id })
