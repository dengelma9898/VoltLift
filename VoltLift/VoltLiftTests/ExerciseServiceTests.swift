...
final class ExerciseServiceTests: XCTestCase {
    ...
    func testGetAllExercises_ReturnsNonEmptyArray() {
        // When
        let exercises = self.exerciseService.getAllExercises()
    ...
    func testGetExercisesForMuscleGroup_WithNoEquipment_ReturnsBodyweightExercises() {
        ...
        // When
        let chestExercises = self.exerciseService.getExercises(for: .chest, availableEquipment: availableEquipment)
        let legExercises = self.exerciseService.getExercises(for: .legs, availableEquipment: availableEquipment)
        let coreExercises = self.exerciseService.getExercises(for: .core, availableEquipment: availableEquipment)

        // Then
        XCTAssertTrue(chestExercises.contains { $0.name == "Push-up" }, "Should contain Push-up for chest")
        XCTAssertTrue(
            legExercises.contains { $0.name == "Bodyweight Squat" },
            "Should contain Bodyweight Squat for legs"
        )
    ...
    func testGetExercisesForMuscleGroup_WithDumbbells_ReturnsAppropriateExercises() {
        ...
        // When
        let chestExercises = self.exerciseService.getExercises(for: .chest, availableEquipment: availableEquipment)
        let armExercises = self.exerciseService.getExercises(for: .arms, availableEquipment: availableEquipment)
        ...
        XCTAssertFalse(
            chestExercises.contains { $0.name == "Dumbbell Bench Press" },
            "Should not contain exercises requiring bench"
        )
    }
    ...
    func testGetExercisesForMuscleGroup_ResultsAreSorted() {
        ...
        // When
        let exercises = self.exerciseService.getExercises(for: .chest, availableEquipment: availableEquipment)
    ...
    func testGetExercisesWithEquipmentHints_SortsAvailableExercisesFirst() {
        ...
        if !availableItems.isEmpty, !unavailableItems.isEmpty {
            XCTAssertLessThan(
                ...
                "Available exercises should come before unavailable ones"
            )
        }
    }
    ...
    func testGetExerciseById_WithValidId_ReturnsCorrectExercise() {
        ...
        // When
        let foundExercise = self.exerciseService.getExercise(by: firstExercise.id)
    ...
    func testGetExerciseById_WithInvalidId_ReturnsNil() {
        ...
        // When
        let foundExercise = self.exerciseService.getExercise(by: invalidId)
    ...
    func testGetExercisesForDifficulty_ReturnsCorrectExercises() {
        ...
        // When
        let beginnerExercises = self.exerciseService.getExercises(
            for: .beginner,
            availableEquipment: availableEquipment
        )
        let advancedExercises = self.exerciseService.getExercises(
            for: .advanced,
            availableEquipment: availableEquipment
        )
        ...
        for exercise in beginnerExercises {
            ...
            XCTAssertTrue(
                ...
                "Should only return exercises with available equipment"
            )
        }
    ...
    func testGetBodyweightExercises_ReturnsOnlyEquipmentFreeExercises() {
        ...
        let exerciseNames = bodyweightExercises.map(\.name)
    ...
    func testGetLegacyExercises_ReturnsCompatibleFormat() {
        ...
        for exercise in legacyExercises {
            ...
            XCTAssertTrue(
                exercise.requiredEquipment.isSubset(of: availableEquipment),
                "Should only return available exercises"
            )
        }
    }
    ...
    func testExerciseCatalogCompatibility_UsesExerciseService() {
        ...
        let serviceExercises = self.exerciseService.getLegacyExercises(
            for: .chest,
            availableEquipment: availableEquipment
        )
        ...
        let catalogNames = Set(catalogExercises.map(\.name))
        let serviceNames = Set(serviceExercises.map(\.name))
    ...
    func testExerciseCatalogEnhanced_ReturnsExerciseDisplayItems() {
        ...
        for item in displayItems {
            ...
            if !item.isAvailable {
                ...
                XCTAssertEqual(
                    item.missingEquipment,
                    expectedMissing,
                    "Missing equipment should be calculated correctly"
                )
            }
        }
    ...
    func testAllExercises_HaveValidMuscleGroups() {
        ...
        for exercise in exercises {
            XCTAssertTrue(
                validMuscleGroups.contains(exercise.muscleGroup),
    ...
    func testExerciseService_HandlesComplexEquipmentFiltering() {
        ...
        // When
        let chestExercises = self.exerciseService.getExercises(for: .chest, availableEquipment: complexEquipment)
        let displayItems = self.exerciseService.getExercisesWithEquipmentHints(
            for: .chest,
            availableEquipment: complexEquipment
        )
        ...
        for exercise in chestExercises {
            XCTAssertTrue(
                exercise.requiredEquipment.isSubset(of: complexEquipment),
    ...
    func testExerciseService_ConsistentSortingBehavior() {
        ...
        for (index, item) in firstCall.enumerated() {
            XCTAssertEqual(
                item.exercise.id,
                secondCall[index].exercise.id,
                "Exercise order should be consistent across calls"
            )
            XCTAssertEqual(
                item.isAvailable,
                secondCall[index].isAvailable,
