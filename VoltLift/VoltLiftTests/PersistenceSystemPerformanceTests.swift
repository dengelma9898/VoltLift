...
final class PersistenceSystemPerformanceTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        userPreferencesService = await UserPreferencesService(persistenceController: persistenceController)
    ...
    func testLargeEquipmentDatasetPerformance() async throws {
        let equipmentCounts = [100, 500, 1000, 2000]
        var results: [Int: (save: Double, load: Double)] = [:]
        
        for count in equipmentCounts {
            // Create large equipment dataset
            let equipment = testDataFactory.createLargeEquipmentSet(count: count)
            ...
            await userPreferencesService.clearEquipmentCache()
            ...
            XCTAssertEqual(userPreferencesService.selectedEquipment.count, count)
            
            // Clean up for next iteration
            try await userPreferencesService.clearAllEquipment()
        }
        ...
        for count in equipmentCounts {
            ...
            print("- \(count) items: Save \(String(format: "%.3f", result.save))s, Load \(String(format: "%.3f", result.load))s")
        }
    }
    ...
    func testLargeWorkoutPlanDatasetPerformance() async throws {
        ...
        for count in planCounts {
            ...
            let plans = testDataFactory.createMultipleWorkoutPlans(count: count)
            ...
            // Clear cache to ensure fresh load
            await userPreferencesService.clearPlanCache()
            ...
            for plan in Array(plans.prefix(10)) {
                _ = try await userPreferencesService.loadPlanDetails(plan.id)
            }
            ...
            // Verify data integrity
            XCTAssertEqual(userPreferencesService.savedPlans.count, count)
            
            // Clean up for next iteration
            try await userPreferencesService.clearAllPlans()
        }
        ...
        for count in planCounts {
            let result = results[count]!
            print("- \(count) plans: Save \(String(format: "%.3f", result.save))s, Load \(String(format: "%.3f", result.load))s, Access \(String(format: "%.3f", result.access))s")
        }
    }
    ...
    func testConcurrentReadPerformance() async throws {
        // Setup test data
        let equipment = testDataFactory.createLargeEquipmentSet(count: 200)
        let plans = testDataFactory.createMultipleWorkoutPlans(count: 100)
    ...
    func testConcurrentWritePerformance() async throws {
        ...
        for concurrency in concurrencyLevels {
            ...
            XCTAssertEqual(userPreferencesService.savedPlans.count, expectedPlanCount, 
            ...
            XCTAssertGreaterThan(userPreferencesService.selectedEquipment.count, 0, 
    ...
    func testMemoryUsageWithLargeDatasets() async throws {
        ...
        // Load large dataset
        let largeEquipmentSet = testDataFactory.createLargeEquipmentSet(count: 1000)
        let largePlanSet = testDataFactory.createMultipleWorkoutPlans(count: 500)
        ...
        await userPreferencesService.optimizeMemoryUsage()
        ...
        print("- After optimization: \(formatBytes(optimizedMemory))")
        print("- Increase: \(formatBytes(memoryIncrease))")
        print("- Reduction: \(formatBytes(memoryReduction))")
    }
    
    /// Tests cache performance and memory management
    func testCachePerformanceAndMemoryManagement() async throws {
        // Create test data
        let plans = testDataFactory.createMultipleWorkoutPlans(count: 200)
        for plan in plans {
            try await userPreferencesService.savePlan(plan)
        }
