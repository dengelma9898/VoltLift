...
final class EquipmentSelectionPersistenceIntegrationTests: XCTestCase {
    ...
    func testEquipmentSelectionPersistence() async throws {
        ...
        let selectedEquipment = savedEquipment.filter(\.isSelected)
        XCTAssertEqual(selectedEquipment.count, 3, "Three items should be selected")

        let selectedNames = Set(selectedEquipment.compactMap(\.name))
    ...
    func testEquipmentSelectionLoadAfterRestart() async throws {
        ...
        let selectedItems = newService.selectedEquipment.filter(\.isSelected)
        XCTAssertEqual(selectedItems.count, 2, "Two items should be selected")

        let selectedNames = Set(selectedItems.map(\.name))
        XCTAssertTrue(selectedNames.contains("Dumbbells"))
    ...
    func testIndividualEquipmentUpdate() async throws {
        ...
        let kettlebellItem = self.userPreferencesService.selectedEquipment.first { $0.id == "kettlebell" }
    ...
    func testSetupCompletionTracking() async throws {
        ...
        XCTAssertFalse(self.userPreferencesService.hasCompletedSetup, "Service should reflect incomplete setup")
        ...
        XCTAssertTrue(self.userPreferencesService.hasCompletedSetup, "Service should reflect complete setup")
    }
    ...
    func testErrorHandlingForCorruptedData() async throws {
        ...
        do {
            ...
            let validEquipment = self.userPreferencesService.selectedEquipment.filter { !$0.id.isEmpty }
    ...
    func testConcurrentEquipmentUpdates() async throws {
        ...
        try await self.userPreferencesService.loadSelectedEquipment()
        let selectedCount = self.userPreferencesService.selectedEquipment.count(where: { $0.isSelected })
    ...
    func testEquipmentSelectionWithLargeDataset() async throws {
        ...
        XCTAssertEqual(self.userPreferencesService.selectedEquipment.count, 100, "All equipment should be loaded")

        let selectedCount = self.userPreferencesService.selectedEquipment.count(where: { $0.isSelected })
        let expectedSelectedCount = largeEquipmentSet.count(where: { $0.isSelected })
    ...
    func testMultipleSaveLoadCycles() async throws {
        ...
        for cycle in 1 ... 5 {
            ...
            // Save and load
            try await self.userPreferencesService.saveEquipmentSelection(currentEquipment)
            try await self.userPreferencesService.loadSelectedEquipment()
            ...
            for equipment in currentEquipment {
                let loadedEquipment = self.userPreferencesService.selectedEquipment.first { $0.id == equipment.id }
                XCTAssertEqual(
