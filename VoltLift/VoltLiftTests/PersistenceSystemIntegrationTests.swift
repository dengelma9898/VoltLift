...
final class PersistenceSystemIntegrationTests: XCTestCase {
    ...
    func testCompleteUserJourney() async throws {
        ...
        let selectedEquipment = testDataFactory.createEquipmentSelection()
        try await userPreferencesService.saveEquipmentSelection(selectedEquipment)
        ...
        let firstPlan = testDataFactory.createWorkoutPlan(name: "My First Workout")
        try await userPreferencesService.savePlan(firstPlan)
        ...
        XCTAssertEqual(userPreferencesService.savedPlans.first?.name, "My First Workout")
        
        // When: User starts a workout using the saved plan
        try await userPreferencesService.markPlanAsUsed(firstPlan.id)
        ...
        let additionalPlans = testDataFactory.createMultipleWorkoutPlans(count: 3)
        ...
        try await userPreferencesService.loadSavedPlans()
        XCTAssertEqual(userPreferencesService.savedPlans.count, 4)
        
        // Most recently used plan should be first
        XCTAssertEqual(userPreferencesService.savedPlans.first?.name, "My First Workout")
        XCTAssertNotNil(userPreferencesService.savedPlans.first?.lastUsedDate)
    }
    ...
    func testEquipmentModificationWorkflow() async throws {
        ...
        let initialEquipment = testDataFactory.createEquipmentSelection()
        ...
        let plansBasedOnEquipment = testDataFactory.createWorkoutPlansForEquipment(initialEquipment)
        for plan in plansBasedOnEquipment {
            try await userPreferencesService.savePlan(plan)
        }
        
        // When: User modifies their equipment selection
        let modifiedEquipment = testDataFactory.modifyEquipmentSelection(initialEquipment)
        try await userPreferencesService.saveEquipmentSelection(modifiedEquipment)
        
        // Then: Equipment changes should be persisted
        try await userPreferencesService.loadSelectedEquipment()
        let loadedEquipment = userPreferencesService.selectedEquipment
        
        let selectedCount = loadedEquipment.filter { $0.isSelected }.count
        let expectedSelectedCount = modifiedEquipment.filter { $0.isSelected }.count
        ...
        try await userPreferencesService.loadSavedPlans()
        XCTAssertEqual(userPreferencesService.savedPlans.count, plansBasedOnEquipment.count)
        
        // When: User creates new plans with modified equipment
        let newPlansWithModifiedEquipment = testDataFactory.createWorkoutPlansForEquipment(modifiedEquipment)
        ...
        XCTAssertEqual(userPreferencesService.savedPlans.count, totalExpectedPlans)
    }
    ...
    func testPlanManagementWorkflow() async throws {
        ...
        let equipment = testDataFactory.createEquipmentSelection()
        try await userPreferencesService.saveEquipmentSelection(equipment)
        
        // And: User has created multiple workout plans
        let originalPlans = testDataFactory.createMultipleWorkoutPlans(count: 5)
        ...
        try await userPreferencesService.loadSavedPlans()
        let renamedPlan = userPreferencesService.savedPlans.first { $0.id == planToRename.id }
        ...
        try await userPreferencesService.loadSavedPlans()
        let usedPlans = userPreferencesService.savedPlans.filter { $0.lastUsedDate != nil }
        ...
        try await userPreferencesService.loadSavedPlans()
        let remainingPlanIds = Set(userPreferencesService.savedPlans.map { $0.id })
        let deletedPlanIds = Set(plansToDelete.map { $0.id })
        
        XCTAssertTrue(remainingPlanIds.isDisjoint(with: deletedPlanIds), "Deleted plans should not exist")
        XCTAssertEqual(userPreferencesService.savedPlans.count, 3, "Should have 3 remaining plans")
    }
    ...
    func testCrossSessionPersistence() async throws {
        ...
        let session1Equipment = testDataFactory.createEquipmentSelection()
        let session1Plans = testDataFactory.createMultipleWorkoutPlans(count: 3)
        
        try await userPreferencesService.saveEquipmentSelection(session1Equipment)
        for plan in session1Plans {
            try await userPreferencesService.savePlan(plan)
        }
        try await userPreferencesService.markPlanAsUsed(session1Plans[0].id)
        ...
        let session1SelectedIds = Set(session1Equipment.filter { $0.isSelected }.map { $0.id })
        let session2SelectedIds = Set(session2Equipment.filter { $0.isSelected }.map { $0.id })
        ...
        let session1PlanIds = Set(session1Plans.map { $0.id })
        let session2PlanIds = Set(session2Service.savedPlans.map { $0.id })
        ...
        let newPlan = testDataFactory.createWorkoutPlan(name: "Session 2 Plan")
        try await session2Service.savePlan(newPlan)
        
        let equipmentUpdate = testDataFactory.createEquipmentItem(id: "new-equipment", isSelected: true)
        ...
        let newEquipmentExists = session3Service.selectedEquipment.contains { $0.id == "new-equipment" && $0.isSelected }
    ...
    func testDataIntegrityMaintenance() async throws {
        ...
        let equipment = testDataFactory.createLargeEquipmentSet(count: 50)
        let plans = testDataFactory.createMultipleWorkoutPlans(count: 20)
        ...
        XCTAssertEqual(userPreferencesService.selectedEquipment.count, equipment.count)
        XCTAssertEqual(userPreferencesService.savedPlans.count, plans.count)
    ...
    func testErrorRecoveryScenarios() async throws {
        ...
        let equipment = testDataFactory.createEquipmentSelection()
        let plans = testDataFactory.createMultipleWorkoutPlans(count: 3)
        ...
        do {
            try await userPreferencesService.renamePlan(nonExistentPlanId, newName: "New Name")
            XCTFail("Should throw error for non-existent plan")
        ...
        do {
            try await userPreferencesService.deletePlan(nonExistentPlanId)
            XCTFail("Should throw error for non-existent plan")
        } catch is UserPreferencesError {
            // Expected error
        }
        ...
        do {
            try await userPreferencesService.renamePlan(plans[0].id, newName: "")
        ...
        try await userPreferencesService.loadSavedPlans()
        let originalPlan = userPreferencesService.savedPlans.first { $0.id == plans[0].id }
    ...
    func testRealisticPerformanceScenarios() async throws {
        ...
        let equipment = testDataFactory.createLargeEquipmentSet(count: 100)
        let plans = testDataFactory.createMultipleWorkoutPlans(count: 50)
