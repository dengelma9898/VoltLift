...
final class EnhancedExerciseSystemIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        metadataService = await ExerciseMetadataService(context: context)
        exerciseService = ExerciseService(metadataService: metadataService)
    }
    ...
    func testExerciseCatalogIntegrationWithService() {
        ...
        let serviceItems = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: availableEquipment)
    ...
    func testLegacyCompatibilityIntegration() {
        ...
        // When
        let legacyExercises = exerciseService.getLegacyExercises(for: legacyMuscleGroup, availableEquipment: availableEquipment)
        let enhancedExercises = exerciseService.getExercises(for: .chest, availableEquipment: availableEquipment)
    ...
    func testComplexEquipmentScenarios() {
        ...
        let scenario1Items = exerciseService.getExercisesWithEquipmentHints(for: .arms, availableEquipment: scenario1Equipment)
        
        // Should have both available and unavailable exercises
        let availableCount = scenario1Items.filter { $0.isAvailable }.count
        let unavailableCount = scenario1Items.filter { !$0.isAvailable }.count
        ...
        let scenario2Items = exerciseService.getExercisesWithEquipmentHints(for: .core, availableEquipment: [])
        let bodyweightItems = scenario2Items.filter { $0.isAvailable }
        ...
        let scenario3Items = exerciseService.getExercisesWithEquipmentHints(for: .legs, availableEquipment: allEquipment)
        
        // All exercises should be available
        let allAvailable = scenario3Items.allSatisfy { $0.isAvailable }
    ...
    func testEquipmentFilteringConsistency() {
        ...
        let firstCall = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: testEquipment)
        let secondCall = exerciseService.getExercisesWithEquipmentHints(for: .chest, availableEquipment: testEquipment)
        ...
        measure {
            for muscleGroup in MuscleGroup.allCases {
                for equipment in equipmentCombinations {
                    _ = exerciseService.getExercisesWithEquipmentHints(for: muscleGroup, availableEquipment: equipment)
                }
            }
        }
    }
    ...
    func testExerciseVariationIntegrity() {
        ...
        for exercise in exercisesWithVariations {
            ...
            let variationNames = exercise.variations.map { $0.name }
            let uniqueNames = Set(variationNames)
            XCTAssertEqual(variationNames.count, uniqueNames.count,
    ...
    func testInvalidInputHandling() {
        ...
        let emptyResults = exerciseService.getExercises(for: .fullBody, availableEquipment: ["NonexistentEquipment"])
        ...
        let bodyweightResults = exerciseService.getExercises(for: .chest, availableEquipment: [])
    ...
    func testExerciseRetrievalEdgeCases() {
        ...
        let invalidId = UUID()
        let result = exerciseService.getExercise(by: invalidId)
