...
final class ExerciseWorkflowIntegrationUITests: XCTestCase {
    ...
    func testExerciseWorkflowWithoutEquipment() throws {
        ...
        let bodyweightIndicators = self.app.staticTexts["Bodyweight"]
        ...
        let needsEquipmentIndicators = self.app.staticTexts.matching(NSPredicate(format: "label BEGINSWITH 'Needs:'"))
        if !needsEquipmentIndicators.isEmpty {
        ...
        let planRow = self.app.staticTexts["Bodyweight Workout"]
    ...
    func testExerciseDetailViewIntegrationWithWorkflow() throws {
        ...
        for section in sectionsToTest {
            let sectionHeader = self.app.staticTexts[section]
    ...
    func testErrorHandlingAndEdgeCases() throws {
        ...
        for muscleGroup in muscleGroups {
            ...
            let hasCells = !exercisesList.cells.isEmpty

            // Either should have exercises or show appropriate message
            XCTAssertTrue(
                hasCells || noExercisesMessage.exists,
        ...
        let exercisesList = self.app.tables.firstMatch
        if !exercisesList.cells.isEmpty {
            let firstCell = exercisesList.cells.firstMatch
            let infoButton = firstCell.buttons["info.circle"]
            infoButton.tap()
    ...
    func testPerformanceWithLargeExerciseList() throws {
        ...
        for muscleGroup in muscleGroups {
            ...
            let exercisesList = self.app.tables.firstMatch
            XCTAssertTrue(
                exercisesList.waitForExistence(timeout: 2),
                "Exercise list should load quickly for \(muscleGroup)"
            )
        }
