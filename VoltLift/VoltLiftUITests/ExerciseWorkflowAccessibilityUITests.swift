...
final class ExerciseWorkflowAccessibilityUITests: XCTestCase {
    ...
    func testEquipmentIndicatorAccessibility() throws {
        ...
        if exerciseCells.count > 0 {
            ...
            let needsEquipmentIndicator = firstCell.staticTexts.matching(NSPredicate(format: "label BEGINSWITH 'Needs:'")).firstMatch
            ...
            if needsEquipmentIndicator.exists {
                XCTAssertTrue(needsEquipmentIndicator.isAccessibilityElement, "Equipment needs indicator should be accessible")
            }
    ...
    func testExerciseDetailViewAccessibility() throws {
        ...
        for header in sectionHeaders {
            let headerElement = app.staticTexts[header]
        ...
        let exerciseIcons = app.images
        if exerciseIcons.count > 0 {
    ...
    func testTargetMusclesAccessibility() throws {
        ...
        let muscleTexts = app.staticTexts.matching(NSPredicate(format: "label CONTAINS 'muscle' OR label CONTAINS 'Muscle' OR label CONTAINS 'Pectoralis' OR label CONTAINS 'Deltoid' OR label CONTAINS 'Triceps'"))
