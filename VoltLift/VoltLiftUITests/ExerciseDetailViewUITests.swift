...
final class ExerciseDetailViewUITests: XCTestCase {
    ...
    func testExerciseDetailViewInformationDisplay() throws {
        ...
        let exerciseIcon = self.app.images.firstMatch
        ...
        let muscleGroupBadges = self.app.staticTexts
            .matching(
                NSPredicate(format: "label IN {'Chest', 'Back', 'Shoulders', 'Arms', 'Legs', 'Core', 'Full Body'}")
            )
        XCTAssertGreaterThan(muscleGroupBadges.count, 0, "Muscle group badge should be displayed")

        // Test section headers
        let descriptionHeader = self.app.staticTexts["Description"]
    ...
    func testTargetMusclesDisplay() throws {
        ...
        let muscleIndicators = self.app.images.matching(NSPredicate(format: "identifier CONTAINS 'circle'"))
    ...
    func testExerciseVariationsDisplay() throws {
        ...
        let variationsHeader = self.app.staticTexts["Variations"]

        if variationsHeader.exists {
            // Verify variation cards are displayed
            let variationCards = self.app.otherElements.matching(NSPredicate(format: "identifier CONTAINS 'variation'"))

            // Verify difficulty indicators for variations
            let difficultyIndicators = self.app.images.matching(NSPredicate(format: "identifier CONTAINS 'circle'"))
            XCTAssertGreaterThan(
                difficultyIndicators.count,
                0,
                "Difficulty indicators should be present for variations"
            )
        }
    }
    ...
    func testEquipmentRequirementsDisplay() throws {
        ...
        self.app.navigationBars.buttons.firstMatch.tap() // Go back
        self.setupEquipment(["Dumbbells"])
        self.navigateToExerciseSelection()
        ...
        if foundEquipmentExercise {
            ...
            let equipmentHeader = self.app.staticTexts["Required Equipment"]
            ...
            let equipmentIcons = self.app.images
                .matching(NSPredicate(format: "identifier CONTAINS 'wrench' OR identifier CONTAINS 'dumbbell'"))
    ...
    func testAddToWorkoutFromDetailView() throws {
        ...
        let exerciseSelectionTitle = self.app.navigationBars["Add Exercise"]
        ...
        let selectedIcon = self.app.images["checkmark.circle.fill"]
